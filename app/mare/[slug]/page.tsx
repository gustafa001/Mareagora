...
export async function generateMetadata({ params }: MareSlugPageProps): Promise<Metadata> {
  ...
  if (!port) {
    return { title: 'Porto não encontrado | MareAgora', description: 'Página de porto não encontrada.' };
  }
...
export default async function MarePage({ params }: MareSlugPageProps) {
  ...
  if (hasCoords) {
    try {
      ...
      if (waveRes.ok && weatherRes.ok) {
        ...
        waveChartData = h.time.slice(startIdx, startIdx + 8).map((t: string, i: number) => ({
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-slate-100">
      ...
      <section className="relative h-[320px] w-full bg-cover bg-center bg-no-repeat"
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-gradient-to-br from-cyan-500/20 to-cyan-600/10 backdrop-blur-md border border-cyan-400/30 rounded-2xl p-5">
              ...
              <div className="text-sm font-bold text-cyan-300 mt-2">Vento: {weatherData.wind} · Ondas: {weatherData.waves}</div>
              <div className="text-xs text-cyan-400/80 mt-1">🌡️ {weatherData.temp}°C · 💧 {weatherData.humidity}% · 📊 {weatherData.pressure} hPa</div>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6">
            ...
            <div className="flex flex-col gap-3">
              {getNearbySlugs(port).map((p: Port) => (
                <Link key={p.slug} href={`/mare/${p.slug}`}
