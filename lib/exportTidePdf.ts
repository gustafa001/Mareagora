...
export async function exportTidePdf({ portName, state, monthLabel, rows, lang = 'pt' }: ExportTidePdfParams): Promise<void> {
  ...
  autoTable(doc, {
    ...
    didParseCell: (data: any) => {
      if (data.section === 'body' && data.column.index >= 1 && data.column.index <= 4) {
        ...
        if (raw.startsWith('A')) data.cell.styles.textColor = [37, 99, 235];
        else if (raw.startsWith('B')) data.cell.styles.textColor = [234, 88, 12];
      }
    },
  });
  ...
  doc.text(
    isEn
      ? 'Local times - A = high tide - B = low tide - Generated by MareAgora (mareagora.com.br)'
      : 'Horarios locais - A = maré alta - B = maré baixa - Gerado por MareAgora (mareagora.com.br)',
    14,
